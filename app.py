...
def iniciar_juego_inicial():
    # Only read from widget keys when called from the "inicio" screen where they are rendered
    st.session_state.cantidad_digitos_total = st.session_state._cantidad_digitos_total
    st.session_state.tiempo_memorizacion = st.session_state._tiempo_memorizacion
    iniciar_juego()
...
if st.session_state.estado == 'inicio':
    ...
    st.number_input("Cantidad total de dígitos", min_value=1, value=st.session_state.cantidad_digitos_total, key="_cantidad_digitos_total")
    st.number_input("Tiempo de visualización (en segundos)", min_value=1, value=st.session_state.tiempo_memorizacion, key="_tiempo_memorizacion")
    st.write("")

    col1, col2, col3 = st.columns([1, 1, 1])
    with col2:
        st.button("Iniciar Juego", on_click=iniciar_juego_inicial, use_container_width=True)
    ...
    # Bucle automático:
    st.write("")
    st.info("Reiniciando en 3 segundos...")
    time.sleep(3) # Aumentado a 3 segundos para que el usuario pueda leer los resultados
